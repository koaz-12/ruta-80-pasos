...
export class SVGBoardRenderer {
    ...
    activateEditorMode() {
        ...
        if (!this.keyboardInitialized) {
            ...
            this.initEditorTools(); // NEW: Init click listeners for tools
    ...
    initEditorTools() {
        this.svg.addEventListener('click', (e) => {
            ...
            if (target) {
                ...
                this.onTileClick(id, e);
                return;
            }

            // If clicking background and tool is "ADD"
            if (this.currentTool === 'add') {
                // Get SVG coordinates
                let point = this.svg.createSVGPoint();
                point.x = e.clientX;
                point.y = e.clientY;
                const loc = point.matrixTransform(this.svg.getScreenCTM().inverse());

                // Add Tile at location
                this.addNewTile(Math.round(loc.x), Math.round(loc.y));
            }
        });
    }
    ...
    onTileClick(id, e) {
        if (this.currentTool === 'connect') {
            if (!this.connectSource) {
                ...
                this.showEditorNotification(`🔗 Origen: ${id} - Selecciona destino`);
                // Highlight source
                const el = this.svg.querySelector(`g[data-id="${id}"]`);
                if (el) el.style.filter = 'drop-shadow(0 0 10px yellow)';
            } else {
                if (this.connectSource === id) {
                    // Cancel
                    this.connectSource = null;
                    this.drawBoard(); // Reset styles
                    this.showEditorNotification('Conexión cancelada');
                    return;
                }
                this.connectTiles(this.connectSource, id);
                this.connectSource = null;
                this.drawBoard();
            }
        } else if (this.currentTool === 'select' || !this.currentTool) {
            this.selectedTileId = id;
            this.showEditorNotification(`👆 Seleccionado: ${id}`);
            // Show Inspector
            this.showTileInspector(id);
        }
    }

    addNewTile(x = 400, y = 700) {
        // Find next ID
        let maxId = 0;
        ...
        this.layoutData.tiles.push(newTile);
        this.render(); // Re-render to show new tile
    ...
    removeTile(id) {
        if (!confirm(`¿Eliminar casilla ${id}?`)) return;
    ...
    drawUnifiedControls() {
        ...
        panel.innerHTML = `
            ...
            <!-- Tools Section (NEW) -->
            <div style="padding:15px; border-bottom:1px solid #333; background:#2a2a2a;">
                <div style="color:#9cdcfe; font-size:13px; font-weight:bold; margin-bottom:12px;">🛠️ HERRAMIENTAS</div>
                
                <div style="display:flex; gap:5px; margin-bottom:15px;">
                    <button id="tool-select" class="tool-btn ${this.currentTool === 'select' ? 'active' : ''}" 
                            style="flex:1; padding:8px; border:none; border-radius:4px; background:${this.currentTool === 'select' ? '#007bff' : '#444'}; color:#fff; cursor:pointer;" title="Seleccionar y Mover">
                        👆 Select
                    </button>
                    <button id="tool-add" class="tool-btn ${this.currentTool === 'add' ? 'active' : ''}" 
                            style="flex:1; padding:8px; border:none; border-radius:4px; background:${this.currentTool === 'add' ? '#28a745' : '#444'}; color:#fff; cursor:pointer;" title="Clic en mapa para añadir">
                        ➕ Añadir
                    </button>
                    <button id="tool-connect" class="tool-btn ${this.currentTool === 'connect' ? 'active' : ''}" 
                            style="flex:1; padding:8px; border:none; border-radius:4px; background:${this.currentTool === 'connect' ? '#fd7e14' : '#444'}; color:#fff; cursor:pointer;" title="Clic origen -> Clic destino">
                        🔗 Conectar
                    </button>
                </div>
        ...
        const setTool = (tool) => {
            ...
            this.drawUnifiedControls();
            this.showEditorNotification(`Herramienta: ${tool.toUpperCase()}`);
        };
        document.getElementById('tool-select').onclick = () => setTool('select');
        document.getElementById('tool-add').onclick = () => setTool('add');
        document.getElementById('tool-connect').onclick = () => setTool('connect');

        // === INSPECTOR LISTENERS ===
        if (this.selectedTileId) {
            const dispInput = document.getElementById('insp-display');
            const typeSelect = document.getElementById('insp-type');
            const delBtn = document.getElementById('insp-delete');

            // Get current values from data
            const tile = this.layoutData.tiles.find(t => t.id === this.selectedTileId);
            if (tile) {
                // Populate if inputs exist (they should)
                ...
                if (typeSelect) typeSelect.onchange = (e) => {
                    tile.type = e.target.value;
                    this.render(); // Need full render for color change
                };
